...
Here we will introduce testing. Save the below code and compile + run it with `zig test file-name.zig`. We will be using the [`expect`](https://ziglang.org/documentation/master/std/#root;testing.expect) function from the standard library, which will cause the test to fail if its given the value `false`. When a test fails, the error and stack trace will be shown.
